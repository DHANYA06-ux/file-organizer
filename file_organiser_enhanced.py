...
def load_config():
    try:
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            # normalize extensions to lower-case sets
...
class FileOrganiserApp:
    def __init__(self, master):
        ...
        master.geometry("680x260")
        master.resizable(False, False)

        self.config = load_config()
        self.moved_records = []  # list of (src, dst) for undo
    ...
    def view_log(self):
        # open log file in system default text viewer
        p = Path(LOG_FILE)
        if not p.exists():
    ...
    def _organise_worker(self, path: Path):
        try:
            ...
            for i, file in enumerate(files, 1):
                ...
                # duplicate detection: compute MD5
                file_hash = compute_md5(file)
                is_duplicate = False
                if file_hash:
                    existing = hash_map.get(file_hash)
                    if existing:
                        # treat as duplicate: move to Duplicates folder
            ...
            # refresh category buttons (maybe new ones)
    ...
    def _show_summary(self, counts: dict, sizes: dict):
        ...
        def show():
            messagebox.showinfo("Organise Summary", "\n".join(text_lines))

        self.master.after(1, show)

    # ---------- Undo ----------
    def undo_last(self):
        # try to read UNDO_TEMP
        p = Path(UNDO_TEMP)
        if not p.exists():
        ...
        for src, dst in reversed(records):
            try:
                ...
                if dstp.exists():
                    # move back only if src parent exists (or recreate)
    ...
    def open_config_editor(self):
        ...
        txt = Text(top)
        # present the current config as JSON for editing
        ...
        def save_and_close():
            try:
                ...
                # normalize to sets of lower-case exts
